...
| ---------------------------- | ---------------------------------------------------- | ------------------------------------- |
| 使用方式                     | [直接開啟網站](https://lyric-flow-seven.vercel.app/) | 安裝後於 `http://127.0.0.1:8765` 使用 |
...
影片目前採即時錄製，錄製所需時間會隨選定區段長度增加。MP4／WebM 格式支援與實際效能取決於瀏覽器；錄製暫存達約 256 MiB 時會停止並保存已錄內容，可降低畫質或縮短匯出區段。
...
瀏覽器開啟 `http://127.0.0.1:8765`。使用期間保留終端機，按 Ctrl+C 關閉服務。
...
辨識與歌詞比對主要針對中文。長音、間奏、重複段落或較強的伴奏都可能影響對齊，結果仍需試聽確認。補辨識與其他程式串接方式見 [API 文件](API.md)。
...
開發模式保留辨識功能，`/api` 會轉接至本機 `http://127.0.0.1:8765`；使用辨識時另開終端機啟動 Python。修改後執行 `npm run build` 更新本機版，或 `npm run build:static` 更新純前端版。
...
也歡迎到 [ReganOba 的 YouTube 頻道](https://www.youtube.com/@ReganOba) 看更多音樂作品。
