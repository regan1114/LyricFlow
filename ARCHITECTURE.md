...
音訊統一由時間軸排程至 Web Audio 主音量節點，預覽與錄影共用輸出，不再維護舊版 HTMLAudioElement 播放路徑。影片由時間軸同步，素材直接持有圖片／影片元素；手動模式的空檔顯示所選內建場景。自動圖片輪播在字幕變更時整理切換節奏，每幀以二分查找定位，避免重複排序與建立歌詞陣列。舊專案中的影片輪播速度及換頁設定會被忽略，素材、時間軸與其他設定照常匯入。
